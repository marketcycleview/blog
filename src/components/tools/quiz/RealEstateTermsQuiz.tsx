...
import { realestateQuizConfig } from "@/lib/tools/quiz/data/realestate-quiz";
import { calculateQuizResult } from "@/lib/tools/quiz/scoring";
import type { QuizResult } from "@/lib/tools/quiz/types";

export default function RealEstateTermsQuiz() {
  const config = realestateQuizConfig;
  ...
  if (result) return (
    <div className="space-y-6">
      <div className={`${result.grade.color} rounded-xl p-6 text-white text-center`}>
        <p className="text-sm opacity-80 mb-1">부동산 상식 등급</p>
      ...
      {result.suggestions.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
          <h3 className="font-bold text-orange-800 mb-2">개선 제안</h3>
    ...
    <div className="space-y-6">
      <div className="bg-white border rounded-xl p-4">
        <div className="flex justify-between mb-2">
          ...
          <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 rounded">{current.category}</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-orange-500 h-2 rounded-full transition-all" style={{ width: `${Math.round((idx / total) * 100)}%` }} /></div>
      </div>
      <div className="bg-white border rounded-xl p-4 sm:p-6">
        ...
        <div className="space-y-3">
          {current.options.map((o, oi) => (
            <button key={oi} onClick={() => handleSelect(o.score)}
              className="w-full text-left px-4 py-3 border-2 rounded-xl hover:border-orange-400 hover:bg-orange-50 transition-colors">
