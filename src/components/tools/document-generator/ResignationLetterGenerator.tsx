...
import { resignationTemplate } from "@/lib/tools/document-generator/templates/resignation";
    ...
    <div className="flex items-center justify-center gap-1 mb-6">
      {steps.map((label, i) => (
        <div key={label} className="flex items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${i < current ? "bg-green-500 text-white" : i === current ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-500"}`}>{i < current ? "✓" : i + 1}</div>
...
function FieldRenderer({ field, value, onChange }: { field: FormField; value: string; onChange: (id: string, v: string) => void }) {
  ...
  if (field.type === "textarea") return (<div><label className="block text-sm font-medium text-gray-700 mb-2">{field.label} {field.required && <span className="text-red-500">*</span>}</label><textarea value={value} onChange={(e) => onChange(field.id, e.target.value)} className={`${base} min-h-[100px]`} placeholder={field.placeholder} rows={4} /></div>);
...
export default function ResignationLetterGenerator() {
  ...
  const template = resignationTemplate;
  ...
  const handleDownload = useCallback(() => {
    ...
    const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `퇴사통보서_${new Date().toISOString().slice(0, 10)}.txt`; a.click(); URL.revokeObjectURL(url);
  }, [doc]);
    ...
    <div className="space-y-6">
      ...
      {!isPreview ? (
        <div className="bg-white border rounded-xl p-4 sm:p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button onClick={handleNext} disabled={!isValid()} className={`flex-1 px-6 py-3 font-bold rounded-lg ${isValid() ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>{step === steps.length - 1 ? "문서 생성" : "다음"}</button>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center"><p className="text-green-800 font-bold text-lg">퇴사 통보서가 생성되었습니다</p></div>
          {doc && <div className="bg-white border-2 border-gray-300 rounded-xl p-6 sm:p-8 font-mono text-sm leading-relaxed whitespace-pre-wrap">{doc.content}</div>}
          <div className="flex flex-wrap gap-3">
            <button onClick={handleCopy} className="flex-1 min-w-[140px] px-6 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">{copied ? "복사 완료!" : "텍스트 복사"}</button>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
        ...
        <ul className="space-y-1 list-disc list-inside">
          <li>근로기준법상 퇴사 통보는 30일 전에 해야 합니다.</li>
          <li>이메일 발송도 유효하지만, 서면 제출이 가장 확실합니다.</li>
          <li>퇴사 후 절차는 퇴사 후 할 일 타임라인을 활용하세요.</li>
        </ul>
      </div>
    </div>
  );
