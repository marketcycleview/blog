...
> **최종 업데이트**: 2026-01-30
> **현재 상태**: Phase 1~4 완료 + Phase 5 일부 (26개 도구 + 5개 엔진)
...
| # | 도구명 | 슬러그 | 난이도 | 카테고리 | 상태 |
|---|--------|--------|--------|----------|------|
| 25 | 오늘의 금리 비교표 | interest-rate-dashboard | 상 | 금융 | ⬜ 미착수 |
| 26 | 전월세 시세 히트맵 | rent-price-heatmap | 상 | 부동산 | ⬜ 미착수 |
| 27 | 지역별 복지 혜택 지도 | regional-welfare-map | 상 | 복지 | ⬜ 미착수 |
| 28 | 창업 상권 분석 지도 | commercial-district-map | 상 | 창업 | ⬜ 미착수 |
| 29 | 자녀 교육비 플래너 | education-cost-planner | 중 | 금융 | ✅ 완료 |
| 30 | 적금vsETFvs부동산 수익비교 | investment-comparator | 중 | 금융 | ✅ 완료 |

```
이 도구들은 외부 데이터(API, 크롤링)에 의존합니다.
사이트가 충분히 성장한 후 개발합니다.

기술 요구사항:
- ISR/cron으로 데이터 주기적 갱신
- 은행 금리: 금융감독원 금융상품통합비교공시 API
- 전월세 시세: 국토교통부 실거래가 공개시스템 API
- 복지 데이터: 복지로 API
- 상권 데이터: 소상공인시장진흥공단 상권정보 API
